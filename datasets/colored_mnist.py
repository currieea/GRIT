    ...
    def _get_data(self):
        ...
        for x, y in test_loader:
            ...
            for d, e in enumerate(self._training_domains):
                ...
                cimages[torch.arange(len(cimages)), (1-colors).long(), :, :] *= 0
                metaarray = torch.stack([3*torch.ones_like(labels), colors, digits[d::2], labels, identity], dim=1)
                mnist_x_list = torch.cat([mnist_x_list, cimages], dim=0)
                mnist_y_list = torch.cat([mnist_y_list, labels], dim=0)
                metaarray_list = torch.cat([metaarray_list, metaarray], dim=0)
            ...
            cimages[torch.arange(len(cimages)), (1-colors).long(), :, :] *= 0
            metaarray = torch.stack([4*torch.ones_like(labels), colors, digits, labels, identity], dim=1)
            mnist_x_list = torch.cat([mnist_x_list, cimages], dim=0)
            mnist_y_list = torch.cat([mnist_y_list, labels], dim=0)
            metaarray_list = torch.cat([metaarray_list, metaarray], dim=0)
...
class ColoredMNISTClipDataset(ColoredMNISTDataset):
    ...
    def _get_data(self):
        ...
        return (
            ...
            torch.load(os.path.join(data_dir, 'y_array.pth')),
            torch.load(os.path.join(data_dir, 'metadata_array.pth'))
        )
