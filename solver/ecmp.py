...
class ECMP(ERM):  
    def __init__(self, hparam):
        ...
        if self.hparam['projection'] == 'oracle':
            ...
            # # idx = np.arange(self.diff.shape[0])
            # # np.random.shuffle(idx)
            # # selected_idx = idx[:int(self.hparam['param2'])]
            # self.diff = self.diff[selected_idx]
        ...
        self.projection = self.cal_project()
    ...
    def fit(self):
        for i in tqdm(range(self.hparam['epochs'])):
            ...
            for x,y_true,metadata in tqdm(self.train_loader):
                ...
                outputs = self.model(x @ self.projection)
    ...
    def cal_project(self):
        U = torch.svd_lowrank(self.diff.T, q=int(self.hparam['param1']))[0]
        return torch.eye(self.diff.shape[1]).to(self.device) - U @ U.T
